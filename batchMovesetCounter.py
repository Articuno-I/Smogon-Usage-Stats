#!/usr/bin/python
# File I/O is going to be the main bottleneck. Doing moveset counting in batch
# (a folder at a time, rather than log by log) should be much more efficient,
# as keylookup.pickle need only be loaded once per run.
...
def movesetCounter(filename, cutoff, teamtype, usage):
        ...
        for moveset in movesets:
            ...
            if 'rating' in moveset.keys():
                if 'rpr' in moveset['rating'].keys() and 'rprd' in moveset['rating'].keys():
                    gxe = victoryChance(moveset['rating']['rpr'],
                            moveset['rating']['rprd'], 1500.0, 130.0)
                    ...
                    if moveset['rating']['rprd'] != 0.0:
                        weight = weighting(moveset['rating']['rpr'],
                                moveset['rating']['rprd'], cutoff)
            ...
            abilities[moveset['ability']] += weight
            ...
            for stat in moveset['evs'].keys():
                ...
                if stat == 'hp':
                    ...
                    n = nmod[moveset['nature']][{'atk': 0, 'def': 1, 'spa': 2,
                            'spd': 3, 'spe': 4}[stat]]
                x = statFormula(baseStats[keyify(species)][stat],
                        moveset['level'], n, moveset['ivs'][stat], ev)

                while ev > 0:
                    if x != statFormula(baseStats[keyify(species)][stat],
                            moveset['level'], n, moveset['ivs'][stat], ev - 1):
                        break
                    ev = ev - 1
    ...
    try:
        ...
        sys.stderr.write('No teammates data for %s (%s)\n'%(filename,
            str(cuttoff))
        teammates = {}
    for s in teammates:
    ...
    if len(gxes) > 0:
        maxGXE = [len(gxes),
            gxes[0],
            gxes[int(math.ceil(0.01 * len(gxes))) - 1],
            gxes[int(math.ceil(0.20 * len(gxes))) - 1]]
    ...
    for x in ['Abilities','Items','Spreads','Moves','Teammates','Checks and Counters']:
        ...
        if x is 'Checks and Counters':
            table = sorted(table,
                    key=lambda table:-(table[1][1] - 4.0 * table[1][2]))
        else:
            table = sorted(table, key=lambda table:-table[1])
        total = 0.0
        for i in range(len(table)): 
            if ((total > .95 and x is not 'Abilities')
                    or (x is 'Abilities' and i > 5)
                    or (x is 'Spreads' and i > 5)
                    or (x is 'Teammates' and i > 11)
                    or (x is 'Checks and Counters' and i > 11)):
                if x is 'Moves':
                ...
                if x is 'Checks and Counters':
                    ...
                    line = u' | %s %6.3f (%3.2f\u00b1%3.2f)'%(table[i][0],
                            100.0 * score, 100.0 * table[i][1][1],
                            100 * table[i][1][2])
                    while len(line) < tablewidth+1:
                        line = line + ' '
                    line = line + ' |\n |\t (%2.1f%% KOed / %2.1f%% switched out)'%(
                            float(100.0 * matchup[0]) / n, float(100.0 * matchup[3]) / n)
                    if float(100.0 * matchup[0]) / n < 10.0:
                        line = line + ' '
                    if float(100.0 * matchup[3]) / n < 10.0:
                        line = line + ' '
                elif x is 'Teammates':
                    line = ' | %s %+6.3f%%'%(table[i][0],
                            100.0 * table[i][1] / count)
            ...
            if ((total > .95 and x is not 'Abilities')
                    or (x is 'Abilities' and i > 5)
                    or (x is 'Spreads' and i > 5)
                    or (x is 'Teammates' and i > 10)
                    or (x is 'Checks and Counters' and i > 10)):
                break
            if x is 'Moves':
...
file = open('Raw/moveset/%s/teammate%s.pickle'%(str(sys.argv[1], specs))
teammateMatrix = pickle.load(file)
file.close()

file = open('Raw/moveset/%s/encounterMatrix%s.pickle'%(str(sys.argv[1]), specs))
encounterMatrix = pickle.load(file)
file.close()

filename = 'Stats/%s.txt'%(str(sys.argv[1]) + specs)
file = open(filename)
table = file.readlines()
file.close()

usage, nBattles = readTable(filename)

pokes = []
for poke in usage.keys():
    pokes.append([poke, usage[poke]])
if sys.argv[1] in ('randombattle', 'challengecup', 'challengecup1v1',
        'seasonal'):
    pokes = sorted(pokes)
else:
...
for poke in pokes:
    if poke[1] < 0.0001: #1/100th of a percent
        break
    stuff = movesetCounter('Raw/moveset/%s/%s'%(str(sys.argv[1]),
                keyify(poke[0])), cutoff, teamtype, usage)
    stuff['usage'] = poke[1]
    chaos['data'][poke[0]] = stuff


filename = 'Stats/chaos/%s.json'%(str(sys.argv[1]) + specs)
