...
def getTeamsFromLog(log, mrayAllowed):
    ...
    for team in ('p1team', 'p2team'):
        ...
        for i in xrange(len(log[team])):
            ...
            while len(moves) < 4:
                moves.append('')
            for j in xrange(len(moves)): #make all moves lower-case and space-free
                old = moves[j]
                try: 
            ...
            if 'hiddenpower' in moves:
                hptype=15 * (ivs['hp'] % 2 + 2 * (ivs['atk'] % 2) +
                        4 * (ivs['def'] % 2) + 8 * (ivs['spe'] % 2) +
                        16 * (ivs['spa'] % 2) + 32 * (ivs['spd'] % 2)) / 63
                moves.remove('hiddenpower')
                moves.insert(0, 'hiddenpower' + ['fighting', 'flying', 'poison',
                    'ground', 'rock', 'bug', 'ghost', 'steel', 'fire', 'water',
                    'grass', 'electric', 'psychic', 'ice', 'dragon', 'dark'][hptype])
...
def LogReader(filename, tier, movesets, ratings):
    ...
    mrayAllowed = tier not in ('ubers', 'battlefactory', 'megamons',
            'gen6ubers', 'gen7ubers', 'gen7pokebankubers')
    ...
    for team in ('p1team', 'p2team'):
        ...
        if len(log[team]) < 6:
            for i in xrange(6 - len(log[team])):
        ...
        if analysis is None:
            sys.stderr.write('Problem with %s\n'%filename)
            return False
        teams[team].append({'bias': analysis['bias'],
            'stalliness': analysis['stalliness'], 'tags': analysis['tags']})
    ...
    if 'log' in log.keys() and tier not in nonSinglesFormats:
        ...
        for line in log['log']:
            ...
            if parsed_line[1] == 'switch' and parsed_line[2].startswith('p1'):
                ...
                try:
                    ...
                    elif (species.endswith('-Mega')
                            or species.endswith('-Mega-X')
                            or species.endswith('-Mega-Y')
                            or species.endswith('-Primal')):
        ...
        for line in log['log'][start:]:
            ...
            if linetype == 'turn':
                ...
                move = line[7 + 5 * spacelog + len(found): string.find(line,
                    "|", 7 + 5 * spacelog + len(found)) - 1 * spacelog]
                if move in ("Roar", "Whirlwind", "Circle Throw", "Dragon Tail"):
                    roar = True
                elif move in ("U-Turn", "U-turn", "Volt Switch", "Baton Pass"):
                    #Should Parting Shot be here? - DoW
                    uturn = True
                ...
                if [ts[11 * (int(line[p]) - 1)][0], species] not in ts:
                    if species == 'Shaymin' and [ts[11 * (int(line[p]) - 1)][0],
                            'Shaymin-Sky'] in ts:
                        ...
                        if not found:
                            ...
                            sys.stderr.write(str([ts[11 * (int(line[p])
                                - 1)][0], species]) + '\n')
                            return False
                active[int(line[p]) - 1] = ts.index([ts[11 * (int(line[p])
                    - 1)][0], species])
                #really, it would be better to go back and revise previous
                #affected matchups, but that be a lot more work
    ...
    i = 0
    while (ts[i][0] == ts[0][0]):
        writeme['p1']['team'].append({'species': ts[i][1], 'KOs': KOs[i],
            'turnsOut': turnsOut[i]})
        i = i + 1
        if i >= len(ts):
    ...
    writeme['p2']['team'] = []
    for j in xrange(i, len(ts)):
        writeme['p2']['team'].append({'species': ts[j][1], 'KOs': KOs[j],
            'turnsOut': turnsOut[j]})
